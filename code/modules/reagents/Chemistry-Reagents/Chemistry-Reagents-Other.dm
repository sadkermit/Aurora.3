...
/singleton/reagent/random
	...
	var/random_property_primary = null
	var/random_property_sideeffect = null
	var/random_property_overdose = null

/singleton/reagent/random/affect_chem_effect(var/mob/living/carbon/M, var/alien, var/removed)
	if(random_property_primary)
		switch(random_property_primary)
			//Beneficial properties
			if(RC_PROPERTY_ANTIEMETIC)
				M.add_chemical_effect(CE_ANTIEMETIC)
			if(RC_PROPERTY_NOITCH)
				M.add_chemical_effect(CE_NOITCH, M.chem_doses[type]*2)
			if(RC_PROPERTY_NOFEVER)
				M.add_chemical_effect(CE_NOFEVER, 5)
			if(RC_PROPERTY_STABLE)
				M.add_chemical_effect(CE_STABLE)
			if(RC_PROPERTY_CLEARSIGHT)
				M.add_chemical_effect(CE_CLEARSIGHT)
			if(RC_PROPERTY_STRAIGHTWALK)
				M.add_chemical_effect(CE_STRAIGHTWALK)
			if(RC_PROPERTY_NOSTUTTER)
				M.add_chemical_effect(CE_NOSTUTTER, 2)
			if(RC_PROPERTY_ANTIPARASITIC)
				M.add_chemical_effect(CE_ANTIPARASITIC, 100)
			if(RC_PROPERTY_ANTIBIOTIC)
				M.add_chemical_effect(CE_ANTIBIOTIC, 30)
			if(RC_PROPERTY_PAINKILLER_WEAK)
				M.add_chemical_effect(CE_PAINKILLER, 30)
			if(RC_PROPERTY_PAINKILLER_MILD)
				M.add_chemical_effect(CE_PAINKILLER, 75)
			if(RC_PROPERTY_PAINKILLER_STRONG)
				M.add_chemical_effect(CE_PAINKILLER, 200) //if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if if
			if(RC_PROPERTY_SPEEDBOOSTER_WEAK)
				M.add_chemical_effect(CE_SPEEDBOOST, 0.5)
			if(RC_PROPERTY_SPEEDBOOSTER_MILD)
				M.add_chemical_effect(CE_SPEEDBOOST, 1)
			if(RC_PROPERTY_SPEEDBOSTER_STRONG)
				M.add_chemical_effect(CE_SPEEDBOOST, 1.5)
			if(RC_PROPERTY_BLOODRESTORER_WEAK)
				M.add_chemical_effect(CE_BLOODRESTORE, 3)
			if(RC_PROPERTY_BLOODRESTORER_MILD)
				M.add_chemical_effect(CE_BLOODSRESTORE, 6)
			if(RC_PROPERTY_BLOODRESTORER_STRONG)
				M.add_chemical_effect(CE_BLOODRESTORE, 10)
			if(RC_PROPERTY_BRAINREGENNER_WEAK)
				M.add_chemical_effect(CE_BRAIN_REGEN, 5)
			if(RC_PROPERTY_BRAINREGENNER_MILD)
				M.add_chemical_effect(CE_BRAIN_REGEN, 20)
			if(RC_PROPERTY_BRAINREGENNERSTRONG)
				M.add_chemical_effect(CE_BRAIN_REGEN, 40)
			//Toxic properties
			if(RC_PROPERTY_NEPHROTOXIN_WEAK)
				M.add_chemical_effect(CE_NEPHROTOXIC, 1*removed)
			if(RC_PROPERTY_NEPHROTOXIN_MILD)
				M.add_chemical_effect(CE_NEPHROTOXIC, 3*removed)
			if(RC_PROPERTY_NEPHROTOXIN_STRONG)
				M.add_chemical_effect(CE_NEPHROTOXIC, 6*removed)
			if(RC_PROPERTY_HEPATOTOXIN_WEAK)
				M.add_chemical_effect(CE_HEPATOTOXIC, 1*removed)
			if(RC_PROPERTY_HEPATOTOXIN_MILD)
				M.add_chemical_effect(CE_HEPATOTOXIC, 3*removed)
			if(RC_PROPERTY_HEPATOTOXIN_STRONG)
				M.add_chemical_effect(CE_HEPATOTOXIC, 6*removed)
			if(RC_PROPERTY_NEUROTOXIN_WEAK)
				M.add_chemical_effect(CE_NEUROTOXIC, 1*removed)
			if(RC_PROPERTY_NEUROTOXIN_MILD)
				M.add_chemical_effect(CE_NEUROTOXIC, 2*removed)
			if(RC_PROPERTY_NEUROTOXIN_STRONG)
				M.add_chemical_effect(CE_NEUROTOXIC, 4*removed)
			if(RC_PROPERTY_CARDIOTOXIN_WEAK)
				M.add_chemical_effect(CE_CARDIOTOXIC, 1*removed)
			if(RC_PROPERTY_CARDIOTOXIN_MILD)
				M.add_chemical_effect(CE_CARDIOTOXIC, 3*removed)
			if(RC_PROPERTY_CARDIOTOXIN_STRONG)
				M.add_chemical_effect(CE_CARDIOTOXIC, 6*removed)
			if(RC_PROPERTY_PNEUMOTOXIN_WEAK)
				M.add_chemical_effect(CE_CARDIOTOXIC, 1*removed)
			if(RC_PROPERTY_PNEUMOTOXIN_MILD)
				M.add_chemical_effect(CE_CARDIOTOXIC, 3*removed)
			if(RC_PROPERTY_PNEUMOTOXIN_STRONG)
				M.add_chemical_effect(CE_CARDIOTOXIC, 6*removed)
			if(RC_PROPERTY_BERSERK)
				if(ishuman(M))
					var/mob/living/carbon/human/H = M
					H.berserk_process()
				M.add_chemical_effect(CE_BERSERK)
				if(M.a_intent != I_HURT)
					M.a_intent_change(I_HURT)
			if(RC_PROPERTY_PACIFIED)
				M.add_chemical_effect(CE_PACIFIED)
			if(RC_PROPERTY_HAUNTED)
				M.add_chemical_effect(CE_HAUNTED, M.chem_doses[type]*2)

	if(random_property_sideeffect)
		switch(random_property_sideeffect)
			if(RC_PROPERTY_EMETIC)
				M.add_chemical_effect(CE_EMETIC, M.chem_doses[type]*2)
			if(RC_PROPERTY_ITCH)
				M.add_chemical_effect(CE_ITCH, M.chem_doses[type]*2)
			if(RC_PROPERTY_FEVER)
				M.add_chemical_effect(CE_FEVER, 4)
			if(RC_PROPERTY_CLUMSY)
				M.add_chemical_effect(CE_CLUMSY)
			if(RC_PROPERTY_DROPITEM)
				M.add_chemical_effect(CE_DROPITEM)

/singleton/reagent/random/overdose(var/mob/living/carbon/M, var/alien, var/removed, var/datum/reagents/holder)
	if(random_property_overdose)
		switch(random_property_overdose)
			//Beneficial properties
			if(RC_PROPERTY_ANTIEMETIC)
				M.add_chemical_effect(CE_ANTIEMETIC)
			if(RC_PROPERTY_NOITCH)
